...
import { ErrorThrow } from '../@types/response.type.d'
...
const idValidator = (req: Request, res: Response, next: NextFunction) => {
  ...
  return responseError(res, new ErrorHandler(STATUS.BAD_REQUEST, error.message))
}
...
const entityValidator = (req: Request, res: Response, next: NextFunction) => {
  ...
  return responseError(
    res,
    new ErrorHandler(STATUS.UNPROCESSABLE_ENTITY, error.message)
  )
}
