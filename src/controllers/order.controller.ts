...
import { PurchaseModel } from '../database/models/order.model'
      ...
      if (item.buy_count > product.quantity) {
        ...
        let data = await PurchaseModel.findOneAndUpdate(
          ...
          },
          {
            ...
            status: STATUS_ORDER.WAIT_FOR_CONFIRMATION,
          },
          {
            new: true,
          }
...
export const getPurchases = async (req: Request, res: Response) => {
  const { status = STATUS_ORDER.ALL } = req.query
  ...
  if (Number(status) !== STATUS_ORDER.ALL) {
